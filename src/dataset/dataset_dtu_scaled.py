...
class DatasetDTU(IterableDataset):
    ...
    def __iter__(self):
        ...
        for idx, meta in enumerate(self.chunks):
            ...
            context_indices, target_indices = np.array([i for i in range(2)]), np.array([i for i in range(2, len(view_ids))])
                ...
                with open(self.pair_filepath) as f:
                    ...
                    for _ in range(num_viewpoint):
                        ...
                        if self.stage != 'train' and len(self.cfg.test_ref_views) > 0:
                            if ref_view not in [23]:
                                continue
                            else:
                                if self.cfg.use_test_ref_views_as_src:
                                    src_views = self.cfg.test_ref_views[1:]
    ...
    def load_cam_info(self):
        for vid in range(self.num_all_imgs):
            ...
            # scale_x = self.cfg.image_shape[1] / self.original_img_wh[1]
            # scale_y = self.cfg.image_shape[0] / self.original_img_wh[0]
    ...
    def define_transforms(self):
        self.transform = T.Compose([T.ToTensor(), T.Resize((self.cfg.image_shape[0], self.cfg.image_shape[1]), antialias=True)])        
